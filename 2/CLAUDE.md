...
**Centre lift (rideable)** — `lifty` bounces between `topy`/`boty` (the top and bottom floor's stand-level, `liftys[]`) at `liftspd` px/frame, pausing for `liftpause` (30 frames, ~1s) whenever it reaches any of the 3 floor-aligned y values (`topy`, `boty`, or the middle floor's `liftys[2]`, checked in `update_room()`'s first block, before anything else runs each frame) — a bug fix; the lift originally never paused at all, just bounced straight through each floor at 1px/frame, so `lift_near`'s already-widened 8px tolerance (see below) only produced a brief few-frame pass-through window at each floor (~10% of a round trip), not a real opportunity to board. `lift_near(fl)` checks alignment within `liftt` px (8; widened from an initial 3, which made the near-window too short to reliably board or dash across) — still used for the boarding/disembark checks below, now backed by the dwell time rather than incidental timing. Walking into the lift's x-column (60-67) while `lift_near(rfl)` boards it (`ronlift=true`, `py` tracks `lifty` every frame); disembarking requires directional input while `lift_near` is true for some floor, snapping `rfl` via `nearest_floor()`.

**Jumping onto the lift (bug fix)** — Landing a jump used to only ever check the gap column at the fixed end of the 24-frame arc (`jt>=jT`), and only to decide whether to trigger `fall_void()`; it never checked whether the lift was actually *there*, so a directed jump (`jdir~=0`) could never land ON the 12px-wide lift platform in the first place — the arc's ~19px horizontal reach is deliberately wider than the gap specifically so it clears the hazard, which structurally means it always overshoots a stationary target inside that same gap. `update_room()`'s `jumping` branch now checks every frame of the arc (not just the last one): if the player's current x crosses the gap column while the lift happens to be aligned (`lift_near(rfl)`), the jump ends immediately and boards the lift (`ronlift=true`, `py=lifty`) right where it caught it, rather than carrying through to the far side. This is what makes "jump onto the lift" possible at all — catching it mid-flight, not landing precisely on it at the arc's end.
...
**Void fall (`fall_void()`)** — walking into the misaligned lift column (not `lift_near(rfl)`) while not invulnerable (`invt<=0`) costs 3 HP and resets the player to this room's entry door (`rfl=2`, `px=entry_side==0 and 116 or 4`, `py=floor_ground(2)-7`), same spawn point `enter_room()` uses on first arrival. The `invt<=0` guard (shared with robot-hit invulnerability) stops a single unbroken hold-toward-the-gap input from re-triggering every frame — after one fall the player gets the same 45-frame grace window a robot hit gives. A landed jump (`jt>=jT` in `update_room()`'s `jumping` branch) checks the same condition against its final `px` before resuming normal standing, and calls `fall_void()` too if it somehow lands inside the gap column — in practice this only matters if the player was already resting in a hazardous column before a straight-up (`jdir==0`) hop, since a directed jump's ~19px horizontal reach always clears the 12px gap with margin (see Jump arc below); it's a correctness backstop for that edge case, not a new way to make jumping risky in the common case. Bug fix: this used to only fire on floors 0-1 (dropping exactly one floor for 3 HP) and was silently blocked on the bottom floor (`rfl==2`) instead of being a hazard there too, and jump never checked its landing spot at all. Both the walk-in and jump-landing paths now go through the same `fall_void()`, on any floor.
