...
**Collision** — Player vs. robot: 1 HP loss plus a small knockback (a few px away from the robot) and a brief invulnerability window (~45 frames) before it can hit again. Player vs. lift shaft edge while not in the lift: 3 HP loss (a fall). Wrong letter-arrangement submission in the control room: 1 HP loss, no retry limit.
