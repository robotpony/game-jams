...
- Search: standing in front of an object and holding up raises the player's hands. A comic-style bubble appears above the player with a progress bar, 0 to 10, about half a second per step. At 10, the object's contents (health or a puzzle letter) are added to the player's inventory and the bubble disappears; finding a letter also shows a brief "found: X" popup with the letter tile. Releasing up (or getting hit) pauses the bar at its current step rather than resetting it; holding up again in front of the same object resumes from there.
...
| ---- | ------ | --------------- |
| Health pickup | Restores 2 HP | A green cross/plus icon shows on the object at all times, so its contents are known before searching; added to inventory once a search completes |
| Puzzle letter | Adds one letter to inventory, toward the secret word (1 of 10 total) | A yellow letter tile shows on the object at all times, so its contents are known before searching; added to inventory once a search completes |
...
| Robot | Red body, dark red legs, white lights | Inverted-U arch on two legs; direction lights blink and shift to whichever side matches its current travel/look direction |
...
| Health pickup | Green | Cross/plus glyph, always shown on top of the object holding it |
| Letter tile | Yellow | Yellow tile badge with the letter printed in black, always shown on top of the object holding it |
