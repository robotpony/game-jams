...
y=0
  ...
  |  a bonus all wave (see Scoring's Planet bonus note). A third,
...
**Planet bonus** — Each of the two entry points (see Enemy entrance above) can be hit by a player shot at any point during the wave, using the same 3px hitbox convention as every other collision check in this doc (`abs(dx)<3 and abs(dy)<3` against `(LX,D1Y)` or `(RX,D1Y)`); a hit awards a flat 200-point bonus, plays a distinct short chime (see Sound below), spawns the same explosion-particle effect as an enemy kill but in yellow rather than the wave's enemy colour, and consumes the shot. Unlike enemies, a planet is never destroyed or removed, has no health, and carries no state at all beyond its fixed position — it can be hit repeatedly across a wave, and across every wave, since it's a fixed landmark the entrance geometry depends on every time `spawn_wave()` runs. **Planets play no part in the wave-clear condition**: that check is `#en==0` alone (see Wave progression above), and a planet is never a member of `en`, so hitting one — zero, one, or a hundred times — neither advances nor delays a wave; it is purely extra score layered on top of the kill-everything win condition, never a second objective. This is a bonus more than a kill: because each entry point sits at roughly the same angular position from the shared centre as the inner-tier enemy band (`~37°`, within the ship's ±69° range but requiring the ship to hold a specific angle rather than fire toward whatever's in front of it), hitting one takes a deliberate, precise shot rather than incidental fire — added after a play-test note asked "you should be able to hit the yellow planets, too."
...
4. **"NEW WAVE n" holds** (`WZ4=30` frames, ~1s) — screen is fully white; text is centred as before but now names the *upcoming* wave (`wv+1`) and is coloured in that wave's own theme colour (`WT[wv%4+1]`, the same 4-wave rotation used for enemies — see Palette), a small preview of what's about to load rather than plain white-on-white text. This is also the point the actual wave-advance state reset happens (`wv+=1`, `en={}`, `eshots={}`, `pt={}`, `ET=0`, `sang=0`, `spawn_wave()`, trail cleared) — before the reveal in stage 5, so there's something already in place underneath it.
...
| Saucer | 10 | The two entry-point sprites at `(32,12)`/`(96,12)` (see Enemy entrance and Planet bonus above) — not a member of `en`, drawn separately |
