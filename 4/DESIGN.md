...
**Entity cap** — Total concurrent enemies + enemy shots is capped at 32. If a wave-scaled dive or fire event would exceed the cap, it's skipped for that frame and retried next opportunity rather than queued, keeping the check O(1). Player shots aren't counted against this cap; the 4-frame cooldown and 4px/frame travel speed already bound them to roughly 8 in flight at once over the 128px playfield.
