...
**Status: not yet built. Phase 0a (asset tooling) done; Phase 0b (design pass) now underway as a proof-of-concept build, running in parallel with pico-8 game 4's own remaining Phase 8.** This is planned as an expanded upscale, not a straight port: bigger playfield/formation, richer sprite art, deeper sound/music, and new mechanics are all in scope, per the design conversation that chose this direction (see [`../../SPEC-FORMAT.md`](../../SPEC-FORMAT.md) for the doc lifecycle this folder follows). The playfield/screen-layout conversation surfaced two findings paper math alone almost missed (see [PLAN.md](PLAN.md)'s Phase 0b intro), so the rest of the design pass proceeds by building a POC in three stages (visuals & layout, movement & waves, transitions) and writing DESIGN.md from what's actually validated, rather than locking every number on paper first. New mechanics stay deferred until after all three stages, evaluated against a real running baseline (default: none, unless one clearly earns its complexity). [DESIGN.md](DESIGN.md) has Stage A's pieces filled in (Screen layout, Palette, Enemy tiers, Horizon/trim rays); State machine, Difficulty ramp, and Token budget stay empty until Stages B/C land. `tools/tic80/sprite_tool.py` exists and is verified (see [`../../tools/TASKS.md`](../../tools/TASKS.md)'s task 5) for whatever sprite work the POC needs.
