...
[`pico8.md`](pico8.md) is the working guide to the whole Claude+Pico-8 workflow across this repo: the document flow, these tools, `lib/`, token/export budgeting, Lua dialect notes, the state machine convention, playtesting technique, and a generalized pitfalls list. This file stays focused on `tools/`'s own scripts and data formats; start at `pico8.md` for anything broader than that. [`tic80.md`](tic80.md) is the equivalent for TIC-80, thinner so far since no TIC-80 game is built yet — see its own Related section for what's still open.
